...
func getproxyfromconfig() string {
	proxy, err := ioutil.ReadFile("proxy.cfg")
	if err != nil {
		fmt.Println(err)
	}
	return string(proxy)
...
func crawlforms(urlval string, finflag chan string) {
	...
	urproxy := getproxyfromconfig()
	rp, err := proxy.RoundRobinProxySwitcher(urproxy)
	if err != nil {
		log.Fatal(err)
	}
	c.SetProxyFunc(rp)
...
func crawllinks(urlval string, finflag chan string) {
	...
	formflag := make(chan string)

	c := colly.NewCollector(
		colly.UserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.246"),
	)

	rp, err := proxy.RoundRobinProxySwitcher("PROXY")
	if err != nil {
		log.Fatal(err)
	}
	c.SetProxyFunc(rp)
	...
	c.OnHTML("a[href]", func(e *colly.HTMLElement) {
		...
		go crawlforms(e.Request.AbsoluteURL(e.Attr("href")), formflag)
		<-formflag
